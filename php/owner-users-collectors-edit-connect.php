<?php
    ...
    if($conn->connect_error) {
        ...
        $stmt_email_check->bind_param("si", $updatedEmailAddress, $collector_id);
        ...
        $stmt_phoneNumber_check->bind_param("si", $updatedPhoneNumber, $collector_id);
       ...
       if($result_email_check->num_rows > 0 && $result_phoneNumber_check->num_rows > 0) {
        ...
        if (!empty($errors)) {
            ...
            $stmt_profile_update = $conn->prepare("UPDATE user_registration SET firstName=?, middleName=?, lastName=?, address=?, phoneNumber=?, emailAddress=? WHERE user_registration_id=?");
            $stmt_profile_update->bind_param("ssssssi", $updateFirstName, $updateMiddleName, $updateLastName, $updateAddress, $updatedPhoneNumber, $updatedEmailAddress, $collector_id);  
            ...
            if (!empty($_FILES['avatar']['name'])) {
                ...
                $updateAvatarQuery->bind_param("si", $avatarContent, $collector_id);
